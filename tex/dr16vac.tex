...
\title{Stellar and substellar companions to APOGEE DR16 stars: \\
...
\begin{equation}
    \sigma_v^2 = (3.5\,(\texttt{VRELERR})^{1.2})^2 + (0.072~\kms)^2
\end{equation}
...
\begin{equation}
    v(t) = K \, h(t \,;\, P, e, \omega, M_0, t_0) + v_0 \label{eq:kepler}
\end{equation}
...
\begin{equation}
    h(t \,;\, P, e, \omega, M_0, t_0) = \cos\left(f + \omega\right) + e\,\cos\omega
\end{equation}
...
samples of interest.
We have rewritten the expression for the marginal likelihood that underlies \thejoker\ in Appendix~\ref{app:marginal-likelihood}, based on the notation in \cite{Hogg:2020}.
...
semi-amplitude with a standard-deviation, $\sigma_K$, that scales with the
period and eccentricity such that
\begin{equation}
    \sigma_K = \sigma_{K, 0} \, \left(\frac{P}{P_0}\right)^{-1/3} \,
        \left(1 - e^2\right)^{-1/2} \label{eq:sigK}
\end{equation}
...
For some science cases and exploration (such as those demonstrated below), it is
useful to filter these stars to generate catalogs of systems with likely
companions, but this requires making decisions and hard cuts based on the
samplings.
However, most of the orbital parameter samplings returned from the procedure
above are highly uncertain and multi-modal, and it is therefore not possible to
define simple cuts on physical parameters like companion mass.
In the previous iteration of this effort, we defined cuts based on percentiles
computed on the distribution of $\ln K$ values after comparing to running the
same pipleline on a control sample of data generated with purely Gaussian noise
properties and time information taken from the \apogee\ \dr{14} visits.
Here, we instead ...

One way of ... likelihood ratio or evidence ratio ...

Describe robust constant fit stuff.

Here we cut on likelihood ratio of binary star model vs. constant model (not fully marginal likelihoods, so cut won't be at 0!).

Other ways one could define a catalog: like we did in paper 1, based on percentiles of, e.g., K
