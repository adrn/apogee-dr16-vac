...
% - Check and cite https://pdfs.semanticscholar.org/64cf/b422cbefe0239fd25ae39cf13833d60a4901.pdf
...
\begin{abstract}\noindent
% Context
Most problems in contemporary astrophysics---from understanding the formation of
black holes to untangling the chemical evolution of galaxies---relies on
knowledge about binary stars.
% Aims
This, in turn, depends on discovery and characterization of binary companions
for large numbers of different kinds of stars across a range of environments.
Contemporary stellar spectroscopic surveys observe hundreds of thousands to
millions of stars, but often with few observational epochs, which makes binary
discovery and orbital characterization challenging.
% Methods
We use a custom Monte Carlo sampler (\thejoker) to perform discovery and
characterization of binary systems by the radial-velocity method, in the
regime of sparse, noisy, and poorly sampled data.
We use it to generate posterior samplings in Keplerian parameters for
\nsources\ sources released in \apogee\ Data Release 16.
% Results
Our final catalog contains \nbinary\ high-confidence systems that show
interesting relationships between binary occurence rate and location in the
color--magnitude diagram.
We find notable faint companions at high masses (black-hole candidates), at low
masses (substellar candidates), and at close separations (mass-transfer
candidates).
We also use the posterior  samplings in a (simple, toy) hierarchical inference
to measure the long-period binary-star eccentricity distribution.
We release the full set of posterior samplings for the entire parent sample of
\nsources\ stars.
This set of samplings involves no heuristic ``discovery''
threshold and therefore can be used for myriad statistical purposes,
including
hierarchical inferences about binary-star populations and sub-threshold searches.
\end{abstract}
...
detailed models of binary star formation and evolution.
...
These surveys are biased towards red-giant stars (although with \dr{16} there
are now many main-sequence stars, see below).
...
To proceed, however, the sampler makes strong assumptions (the source is a
...
Importantly, we use \thejoker\ to sample \emph{every} target as if it were a
SB1.
Discovery of which stars really do have companions then becomes a
post-processing step on the confidence of the characterization; That is, this
project deliberately conflates discovery with characterization.
...
Like the compilation of companion frequencies shown in \cite{Duchene:2013}, we
find that the binary fraction increases steeply for primary stellar masses $M_1
\gtrsim 1.1~\msun$ ($\Teff \gtrsim 6000~\textrm{K}$).
...
significantly anti-correlated with metallicity \citep[e.g.,][]{Moe:2019}.
...
\subsection{Sub-threshold searches}
\label{sec:subthreshold}

\todo{APW TODO: Feel free to delete this! But here is where we could say
something like this:}
One problem with a traditional catalog of detected binary companions
(like our Table~whatever) is that there are many sources in which the
evidence for a binary is significant, but not strong enough to qualify
as a reliable detection.
A user who has informative data on a star that would take a
sub-threshold source above threshold has no recourse if only given a
rigidly thresholded catalog.
Even for well detected binary systems, in a traditional catalog of
orbital parameters, there is no simple way to combine the results with
new data to improve or adjust parameter estimates.
The samplings provided here (in Table~whatever) can be used to solve
these problems; they can be used to combine the \apogee\ information
with new data, without requiring an ab initio re-analysis of
the original \apogee\ data.

As an example, we do APW HELP ME... maybe take a sub-threshold star and
imagine that we got an RV point at some time in the future and show that
it jumps above threshold?? It's just a cutesie example that would involve
importance sampling the sampling with the likelihood from the new point.

blah blah blah...

The ability to perform sub-threshold searches or add external
information is limited by detailed shape of the posterior pdf and the
number of samples we deliver.
If the new data are highly informative, or favor a mode in the
posterior pdf that is not well sampled, the 512-sample posterior
samplings we deliver will not be dense enough to provide support for
the updated posterior pdf.
This puts limitations on the scope of sub-threshold searches, but at
least they are possible with these outputs, in some regime of applicability.
...
being hierarchical triple-star systems, but this is beyond the scope of the
general-use catalog considered here.

\todo{I (HOGG) think we should NOT promise future work. So soften that ``we will generate'' above.}

\todo{Another assumption is that the binaries are single-lined.}
Some are \emph{in fact} double lined and we are definitely doing those wrong.
We must be getting biased results.
Also many will be in an intermediate state, where they aren't double lined but
you can detect the second spectrum. CITE EL~BADRY.
These will also be biased.
...
For one, the binary fraction increases rapidly with stellar mass and with
decreasing metallicity.
\todo{See Moe paper for discussion of this?}
- Dark matter masses of dwarfs??
- Impacts on stellar population evolution - e.g., how does varying binary fraction change chemical enrichment?

We also do not observe systems with evolved-star members that have short-period
companions that could have survived the common-envelope phase.
Combined with the first point, this implies that some appreciable fraction of
evolving stars (on their way to the RGB or horizontal branch) likely merge with
their companion.
If true, ... do the merger stars move to other places in the CMD? Bias stellar mass inferences for old stellar populations using SSP models?

These undoubtebly also ... constrain binary star formation physics ...

% \todo{Implications / theoretical understanding of this. See Moe paper.}

\todo{Mention here that any real understanding of the binary fraction will
require a model of the selection function, both for \apogee\ and for our detections. This then
...
\todo{We have done nothing to deliver completeness or false-positive estimates.}
I (HOGG) don't think we need to do this in this paper; it is out of scope.
However, it is critical for use of our 20,000 objects as a catalog for performing
statistical tests.
For example, if we want to interpret the beautiful binary fraction plot!
In general, completeness will be a function of amplitude, period,
orbital phase, and eccentricity vector (maybe in that order).

Because we have released all our code open-source, we have made it very easy
for anyone to do this kind of work.
Here's a little outline of how one might do it, and what one might find.
...
\begin{description}
    \item[Signatures of companion engulfment] at the red clump, and with decreasing logg
    \item[The binary fraction is anti-correlated with metallicity]
    \item[Brown dwarfs]
    \item[Non-interacting compact object candidates]
    \item[Eccentricity distribution]
    \item[Sub-threshold search and discovery]
