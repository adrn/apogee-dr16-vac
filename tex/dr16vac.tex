...
\begin{abstract}\noindent
% Context
Most problems in contemporary astrophysics---from understanding the formation and
occurrence rate of stellar-mass black holes to untangling the chemical
evolution of galaxies---relies on inferences about binary-star populations.
% Aims
These inferences depend on discovery and characterization of binary companions for
...
often with few, sparse (in time) observations that make companion
...
of binary systems by the radial-velocity method.
The method works with sparse, noisy, and poorly sampled radial-velocity information.
We use it to generate posterior samplings in Keplerian orbital parameters for
...
% Results
Our final catalog contains \nbinary\ systems, with large numbers on both the main
sequence and the red-giant branch.
The catalog shows interesting relationships between
...
We also use these samplings in a (simple, toy) hierarchical inference to
measure the long-period binary-star eccentricity distribution.
We also release the full set of posterior samplings (512 samples per source,
assuming each source is a single-lined spectroscopic binary) for the entire parent
sample of \nsources stars; this set of samplings involves no heuristic ``discovery''
threshold and therefore can be used for myriad statistical inferences.
\end{abstract}
...
% No Gaia sources within 2" and 5 mags
% No Gaia sources within 4" and brighter
...
\begin{figure}[!t]
    \begin{center}
    \includegraphics[width=0.6\textwidth]{eccentricity-distr.pdf}
    \end{center}
    \caption{%
    TODO
    \label{fig:eccdist}
