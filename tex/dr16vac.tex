...
define simple cuts on physical parameters like companion mass to produce a
simple catalog.
...
\apogee\ \dr{14} visits.
Here, we instead compute a marginal likelihood ratio and define a cut

In addition to producing posterior samplings in Keplerian orbital parameters for each \apogee\ source, we also


comparing the two-body
orbit model (assumed by \thejoker) to a constant-velocity fit to each \apogee\
source's visit velocity data.
